/*
 * Copyright (C) 2016 Sebastiano Poggi
...
public class DeviceArtDescriptor {
        ...
        for (File file : files) {
            try {
                ...
                if (document != null) {
                    ...
                    Logger.getInstance().error("Couldn't parse " + file);
                }
            } catch (IOException e) {
    ...
    private static File getBundledDescriptorsFolder() {
        ...
        String[] paths = new String[]{
                String.format("plugins%1$sandroid%1$s", File.separatorChar),
                String.format("plugins%1$sandroid%1$slib%1$s", File.separatorChar),
                String.format("..%1$sadt%1$sidea%1$sandroid%1$s", File.separatorChar),
                String.format("android%1$sandroid%1$s", File.separatorChar),
                String.format("community%1$sandroid%1$sandroid%1$s", File.separatorChar),
        };
    ...
    private static String getBasePath() {
        String studioHome = System.getenv("FRAMER_STUDIO_HOME");
        if (studioHome == null) {
            throw new IllegalStateException("FRAMER_STUDIO_HOME not set!");
        }
        return studioHome;
    }
    ...
    private static void addDescriptors(List<DeviceArtDescriptor> result, Document document, File baseFolder) {
    ...
    private double getAspectRatio(ScreenOrientation orientation) {
    ...
    /**
     * Descriptor for a particular device frame (e.g. a set of images for a particular device in a particular
     * orientation)
     */
    private static class OrientationData {
        ...
        File getBackgroundFile() {
            return !isEmpty(myBackgroundName) ? new File(myDevice.getBaseFolder(), myBackgroundName) : null;
        }

        @Nullable
        File getShadowFile() {
            return !isEmpty(myShadowName) ? new File(myDevice.getBaseFolder(), myShadowName) : null;
        }

        @Nullable
        File getReflectionFile() {
            return !isEmpty(myReflectionName) ? new File(myDevice.getBaseFolder(), myReflectionName) : null;
        }

        @Nullable
        File getMaskFile() {
            return !isEmpty(myMaskName) ? new File(myDevice.getBaseFolder(), myMaskName) : null;
        }
